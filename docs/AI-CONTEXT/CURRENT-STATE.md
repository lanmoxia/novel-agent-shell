...
- 当前校验链路已覆盖教辅模板结构、教辅字段、真实教辅项目实例和生产稿包边界；真实教辅烟测项目已创建并补齐 Phase 0-4 文件，且已通过匹配其语义的校验。Phase 5 教学与生产审查已经完成，Phase 6 轻量修订及复核已经完成，Phase 7 锁稿前审查已经完成，Phase 8 锁稿计划已经制定；当前主要问题是等待用户确认是否执行 Phase 8 锁稿，且当前不进入分镜提示词或 Seedance 提示词测试。
...
建议下一步执行 `BACKLOG.md` 中的 `WENYAN-SMOKE-009-PHASE8-RUN`：按用户确认范围执行 Phase 8 锁稿。

当前不得自动执行 Phase 8；后续必须等待用户确认后，才可生成锁稿相关文件。当前主线只验证剧本创作链路，不测试分镜提示词、Seedance2.0 提示词或视频生产链路。
