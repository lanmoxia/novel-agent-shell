...
- 第二篇文言文泛化测试已执行到 Phase 4：测试文本为《司马光》，项目目录为 `projects/wenyan-generalize-sima-guang-001/`，当前状态是 `phase4_smoke_draft`。该测试正在验证 `wenyan-skill` 能否从《两小儿辩日》的争辩型 / 观点证据型结构泛化到事件危机型 / 行动解困型结构；当前只能写成第二篇泛化验证进行中，不能写成 `wenyan-skill` 已全泛化验证通过。
...
- 不得把《司马光》写成已锁稿项目、已完成正式项目或 `wenyan-skill` 已全泛化验证通过；当前只完成到 Phase 4 烟测草案，Phase 5 审查必须等待用户确认。
- 不得把教辅短剧规则硬编码进壳子主流程。
