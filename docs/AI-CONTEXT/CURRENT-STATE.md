...
- 第二篇文言文泛化测试已执行到 Phase 5：测试文本为《司马光》，项目目录为 `projects/wenyan-generalize-sima-guang-001/`，当前状态是 `phase5_reviewed`。Phase 5 审查结论为 P0=0、P1=0、P2=6，允许在用户确认后进入 Phase 6 轻量修订。该测试正在验证 `wenyan-skill` 能否从《两小儿辩日》的争辩型 / 观点证据型结构泛化到事件危机型 / 行动解困型结构；当前只能写成第二篇泛化验证进行中，不能写成 `wenyan-skill` 已全泛化验证通过。
...
- `WENYAN-GENERALIZE-003-REVISION-PLAN`：如用户希望先拆解 P2 修订项，先制定修订计划。
...
- 不得把《司马光》写成已锁稿项目、已完成正式项目或 `wenyan-skill` 已全泛化验证通过；当前只完成到 Phase 5 教学与生产审查，Phase 6 轻量修订必须等待用户确认。
- 不得把教辅短剧规则硬编码进壳子主流程。
