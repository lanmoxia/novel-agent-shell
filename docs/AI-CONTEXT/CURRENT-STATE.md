...
- `WENYAN-GENERALIZE-001-RUN` 已完成，已创建《司马光》泛化测试项目 `projects/wenyan-generalize-sima-guang-001/` 并推进到 Phase 4 烟测草案，归档到 `docs/AI-CONTEXT/archive/WENYAN-GENERALIZE-001-RUN.md`；本轮未进入 Phase 5/6/7/8，未锁稿，未生成正式字幕、正式分镜、Seedance prompts、Seedance2.0 prompt、视频提示词、图像提示词或生产稿包。
- `WENYAN-GENERALIZE-002-PHASE5-REVIEW` 已完成，已生成《司马光》Phase 5 教学与生产审查报告并归档到 `docs/AI-CONTEXT/archive/WENYAN-GENERALIZE-002-PHASE5-REVIEW.md`；审查结论为 P0=0、P1=0、P2=6，允许在用户确认后进入 Phase 6 轻量修订；本轮未修订 `SMOKE-DRAFT.md`，未进入 Phase 6/7/8，未锁稿，未生成正式字幕、正式分镜、视频提示词、Seedance prompts、Seedance2.0 prompt、图像提示词或生产稿包。
...
- 第二篇文言文泛化测试已执行到 Phase 6：测试文本为《司马光》，项目目录为 `projects/wenyan-generalize-sima-guang-001/`，当前状态是 `phase6_revised`。Phase 6 已处理观看张力、儿童化判断、群儿调度、安全化表达和学习闭环相关 P2，仍保留教材版本、目标年级、最终时长和原文采用版本正式化前待确认。该测试正在验证 `wenyan-skill` 能否从《两小儿辩日》的争辩型 / 观点证据型结构泛化到事件危机型 / 行动解困型结构；当前只能写成第二篇泛化验证进行中，不能写成 `wenyan-skill` 已全泛化验证通过。
...
- `WENYAN-GENERALIZE-003B-PHASE6-RECHECK`：如用户希望先复核修订质量，可先做 Phase 6 复核。
...
- 不得把《司马光》写成已锁稿项目、已完成正式项目或 `wenyan-skill` 已全泛化验证通过；当前只完成到 Phase 6 轻量修订，Phase 7 锁稿前审查必须等待用户确认。
- 不得把教辅短剧规则硬编码进壳子主流程。
